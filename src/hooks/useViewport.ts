...
export function useViewport(elRef: React.RefObject<HTMLElement>) {
  ...
  const centerOn = useCallback((px: number, py: number, containerW: number, containerH: number) => {
    setPan({ x: containerW / 2 - px * zoom, y: containerH / 2 - py * zoom })
  }, [zoom])

  return { zoom, setZoom, pan, setPan, reset, centerOn, onMouseDown, onMouseMove, onMouseUp, onTouchStart, onTouchMove }
}
